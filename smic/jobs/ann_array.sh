...
sed -i "s#NtileJ ==.*#NtileJ == 16#" ${coawstproj}/ocean.in 
...
#   if [ ! -f "${coawstproj}/coawstM" ]; then #test if compilation successful
      ...
      qsub -e ${coawstproj}/coawst.e -o ${coawstproj}/coawst.o -v coawstproj,nprocs -W depend=afterok:$coawstcompile coawst_job.pbs
      wait ${!}
      sleep 1
#   else
