...
sed -i "s#NtileJ ==.*#NtileJ == 22#" ${annproj}/ocean_ann.in 
...
sed -i "s#COMPUTE NONSTATIONARY 20000101.000000 600 SEC.*# COMPUTE NONSTATIONARY 20000101.000000 600 SEC 20000101.010000#" ${annproj}/swan_ann.in #change to 30 day run
...
sed -i "s#RSTNAME ==.*#RSTNAME == ${annproj}/results/ocean_rst_ann.nc#" ${annproj}/ocean_ann.in #replace restart file output location
...
cp ${annproj}/ocean_rst_orig.nc ${annproj}/ocean_ini.nc #copy restart file to init file
...
#anncompile=$(sbatch --output=${annproj}/compile.out --error=${annproj}/compile.err --export=ALL --parsable ann_compile.sh)
#sbatch --dependency=afterok:$anncompile --output=${annproj}/ann.out --error=${annproj}/ann.err --export=ALL,annproj=${annproj} annulus_job.sh
