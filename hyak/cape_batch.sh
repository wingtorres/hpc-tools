...
#dirname="dSdz_${dSdz}_H_${Hs}_D_${direction}_a_${amp}_f_${phi}"
#dirname="dSdz_${dSdz}_H_${Hs}_D_${direction}_v_${v}_bathy_multi"
dirname="dSdz_${dSdz}_H_${Hs}_D_${direction}_v_${v}_bathy_3h"
export coawstproj="/gscratch/nearshore/wtorres/${dirname}"

#Copy directory over
#cp -a ${homeproj}/. ${coawstproj} #copy all input/grid files to output folder
rsync -av ${homeproj}/ ${coawstproj} --exclude=coawstM --exclude=coawstG --exclude=Build* --exclude=grids/
...
#waves
sed -i "s#PAR .*#PAR ${Hs} 6.28 ${direction} 10.0#g" ${coawstproj}/swan.in
...
#velocity
sed -i "s#v0=.*#v0=${v}_r8#" ${coawstproj}/ana_m2obc.h
...
#sbatch --output=${coawstproj}/coawst.out --error=${coawstproj}/coawst.err --export=coawstproj=$coawstproj coawst_job.slurm
